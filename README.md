...
- Tokens are used for authentication. When entering an application, the server protects users from accessing only their own data. A web token is used during all request with the web server so that the server can respond with the users secure data.

2. What steps can you take in your web apps to keep your data secure?

- I can make data private to other users, and only allow webtokens to be used to authenticate access to user data

3. Describe how web servers work.

- Web servers are the computers that store all our data. Servers are the backend of to the frontend of web development. Servers respond the request of clients. A client can send a server a request to preform a certian operation (CRUD) on the database that the server stores.
